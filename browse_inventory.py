...
def restore_command_groups(inventory_path, inventory, dir_prefixes, file_paths,
    ...
    """
    Build the restore command as a list of shell-quoted "groups" (each group
    is one flag plus its value, or the leading interpreter+script), meant to
    be joined with " \\\n  " for display/writing. Shared by
    build_restore_script() and the 'd' debug view so they can't disagree
...
def build_restore_script(inventory_path, inventory, dir_prefixes, file_paths, whole_tree,
    ...
    cmd_groups = restore_command_groups(
        inventory_path, inventory, dir_prefixes, file_paths, restore_dir, script_dir, verbose,
    )
    lines.append(" \\\n  ".join(cmd_groups))
    lines.append("")
    return "\n".join(lines)
...
def _show_debug_view(stdscr, objects, command_lines):
    """
    Full-screen scrollable listing of the archive objects for the current
    selection, plus the restore command that selection would currently
    produce (assuming the original restore location; command_lines is None
    ...
    while True:
        ...
        row = 1
        if command_lines:
            label = " Restore command (assumes original location; 'w' lets you pick a new one):"
            stdscr.addnstr(row, 0, label[:width - 1], width - 1, curses.A_BOLD)
            row += 1
            shown = command_lines[:max_cmd_rows]
            for line in shown:
                text = "   " + line
                stdscr.addnstr(row, 0, text[:width - 1], width - 1)
                row += 1
            remaining = len(command_lines) - len(shown)
            if remaining > 0:
                text = f"   ... ({remaining} more line(s); see 'w' for the full command)"
                stdscr.addnstr(row, 0, text[:width - 1], width - 1)
                row += 1
        else:
            text = " Restore command: (nothing marked yet)"
            stdscr.addnstr(row, 0, text[:width - 1], width - 1)
            row += 1
        row += 1  # blank separator before the object list
        ...
        for r in range(list_height):
            ...
            stdscr.addnstr(list_top + r, 0, line[:width - 1], width - 1, attr)
...
class App:
    ...
    def show_debug(self):
        ...
        command_lines = None
        if self.marks:
            dir_prefixes, file_paths, _whole_tree = compute_restore_selection(self.marks)
            script_dir = os.path.dirname(os.path.abspath(__file__))
            cmd_groups = restore_command_groups(
                self.inventory_path, self.inventory, dir_prefixes, file_paths,
                None, script_dir, verbose=self.verbose_script,
            )
            command_lines = " \\\n  ".join(cmd_groups).split("\n")
        _show_debug_view(self.stdscr, objects, command_lines)
    ...
    def write_script_flow(self):
        ...
        inventory_id = self.inventory.get("inventory_id", "selected")
        default_script = os.path.join(os.getcwd(), f"restore_{inventory_id}.sh")
